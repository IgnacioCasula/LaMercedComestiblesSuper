...
function cargarProveedores() {
    const tb = byId('proveedores-body');
    tb.innerHTML = '';
...
function cargarMovimientos() {
    const tb = byId('movimientos-body');
    tb.innerHTML = '';
...
function guardarProducto() {
    const idValue = byId('producto-id').value;
    const obj = {
    ...
    // CORRECCIÓN: Verificar correctamente si es edición o creación
    if (idValue && idValue !== "") {
        const id = parseInt(idValue);
        const i = productos.findIndex(p => p.id === id);
        if (i > -1) {
            productos[i] = { ...productos[i], ...obj };
...
function eliminarProveedor(id) {
    if (productos.some(p => p.proveedor === id)) {
        return alert('No se puede eliminar: tiene productos asociados.');
    }
...
function guardarProveedor() {
    const idValue = byId('proveedor-id').value;
    const obj = {
    ...
    if (idValue && idValue !== "") {
        ...
        if (i > -1) {
            proveedores[i] = { ...proveedores[i], ...obj };
...
function eliminarCategoria(id) {
    if (productos.some(p => p.categoria === id)) {
        return alert('No se puede eliminar: tiene productos asociados.');
    }
...
function guardarCategoria() {
    const idValue = byId('categoria-id').value;
    const obj = {
...
function openModal(type) {
    ...
    if (type === 'producto') {
        if (!byId('producto-id').value) {
            byId('modal-producto-title').textContent = 'Nuevo Producto';
            byId('form-producto').reset();
        }
    }
    if (type === 'proveedor') {
        if (!byId('proveedor-id').value) {
            byId('modal-proveedor-title').textContent = 'Nuevo Proveedor';
            byId('form-proveedor').reset();
        }
    }
    if (type === 'categoria') {
        if (!byId('categoria-id').value) {
            byId('modal-categoria-title').textContent = 'Nueva Categoría';
...
function closeModal(type) {
    ...
    // Limpiar campos ID al cerrar el modal
    if (type === 'producto') byId('producto-id').value = '';
    if (type === 'proveedor') byId('proveedor-id').value = '';
    if (type === 'categoria') byId('categoria-id').value = '';
}
...
window.onclick = function(e) {
    document.querySelectorAll('.modal').forEach(m => {
        if (e.target === m) {
            ...
            // Limpiar campos ID al hacer clic fuera del modal
            byId('producto-id').value = '';
            byId('proveedor-id').value = '';
            byId('categoria-id').value = '';
        }
    });
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (fProd) fProd.addEventListener('submit', e => { e.preventDefault(); guardarProducto(); });
    
    const fProv = byId('form-proveedor');
    if (fProv) fProv.addEventListener('submit', e => { e.preventDefault(); guardarProveedor(); });
    
    const fCat = byId('form-categoria');
    if (fCat) fCat.addEventListener('submit', e => { e.preventDefault(); guardarCategoria(); });
    
    const fMov = byId('form-movimiento');
    if (fMov) fMov.addEventListener('submit', e => { e.preventDefault(); guardarMovimiento(); });
...
function solicitarProducto(id) {
    alert('Solicitud de reposición enviada para el producto ID: ' + id);
}