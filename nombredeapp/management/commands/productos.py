    ...
    def handle(self, *args, **options):
        ...
        for nombre, marca, precio, categoria_nombre, codigo_barras in productos_data:
            try:
                ...
                producto_existente = Productos.objects.filter(codigobarraproducto=codigo_barras).first()
                
                if producto_existente:
                    # Actualizar producto existente
                    producto_existente.nombreproductos = nombre
                    producto_existente.marcaproducto = marca
                    producto_existente.precioproducto = precio
                    producto_existente.idcategoria = categorias[categoria_nombre]
                    producto_existente.save()
                    productos_actualizados += 1
                    self.stdout.write(self.style.WARNING(f'🔄 Producto actualizado: {nombre} - ${precio}'))
                else:
        ...
        self.stdout.write(self.style.SUCCESS(f'🎉 Se crearon {productos_creados} productos nuevos y se actualizaron {productos_actualizados} productos existentes!'))
