class GestorVenta {
    ...
    agregarProducto() {
        ...
        // Mostrar vista previa del producto agregado
    ...
    crearFilaProducto(tablaBody, productoId, producto) {
        ...
        // ✅ AGREGAR EVENTOS HOVER PARA VISTA PREVIA
        nuevaFila.addEventListener('mouseover', () => {
            this.mostrarVistaPrevia(productoId, producto);
        });
    ...
    // ✅ RECARGO AUTOMÁTICO POR CRÉDITO
    ...
    // ✅ NUEVA FUNCIÓN: LIMPIAR VISTA PREVIA
    limpiarVistaPrevia() {
        const vistaPrevia = document.getElementById('vistaPreviaProducto');
        if (vistaPrevia) {
            vistaPrevia.innerHTML = `
    ...
    generarPDF(resultadoVenta) {
        ...
        if (!ticketItems || ticketItems.children.length === 0) {
            console.error('❌ El ticket no tiene items.');
            return;
        }
